local MAJOR, MINOR = "LibPetBreedInfo-1.0", 31.7 -- A non-official version by Jaliborc
local lib, oldminor = LibStub:NewLibrary(MAJOR, MINOR)

if not lib then return end

lib.breedData = {}
lib.battle = lib.battle or {}
lib.frame = lib.frame or CreateFrame('Frame')
lib.frame:UnregisterAllEvents()
lib.frame:RegisterEvent('PET_BATTLE_CLOSE')
...
local epsilion = .005
local STATE_SpeedPercent = 25
local STATE_MaxHealthBonus = 2
local STATE_MaxHealthPercent = 99
...
function lib:GetBreedByPetBattleSlot(owner, i)
	if not C_PetBattles.IsInBattle() or
		(owner ~= LE_BATTLE_PET_ALLY and owner ~= LE_BATTLE_PET_ENEMY) or
		(i <= 0 or i > MAX_PETS_PER_TEAM and i <= C_PetBattles.GetNumPets(owner)) then return end

	local cache = self.battle[owner] or {}
	self.battle[owner] = cache

	if not cache[i] then
		local speedScale  = C_PetBattles.GetStateValue(owner, i, STATE_SpeedPercent)
		local healthScale = C_PetBattles.GetStateValue(owner, i, STATE_MaxHealthPercent)
		local healthBonus = C_PetBattles.GetStateValue(owner, i, STATE_MaxHealthBonus)

		if speedScale and healthScale and healthBonus then
			speedScale  = inverseStats(speedScale)
			healthScale = inverseStats(healthScale)

			if C_PetBattles.IsWildBattle() and owner == LE_BATTLE_PET_ENEMY then
				healthScale = healthScale * WILD_PET_HEALTH_MULTIPLIER
			end

			local specie = C_PetBattles.GetPetSpeciesID(owner,i)
			local speed  = C_PetBattles.GetSpeed(owner,i)
			local power  = C_PetBattles.GetPower(owner,i)
			local health = C_PetBattles.GetMaxHealth(owner,i) 
			local rarity = C_PetBattles.GetBreedQuality(owner,i)
			local level  = C_PetBattles.GetLevel(owner,i)

			cache[i] = {self:GetBreedByStats(specie, level, rarity,
				health * healthScale - healthBonus,
				power, round(speed * speedScale))}
		end
	end

	return unpack(cache[i] or {})
end
